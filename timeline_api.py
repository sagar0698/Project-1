...
def getHomeTimeline():
    ...
    conn.row_factory = dict_factory
    cur = conn.cursor()
    homeTweets = cur.execute('SELECT TWEET, DAY_OF, FK_USER FROM TWEETS INNER JOIN FOLLOW ON FOLLOW.FOLLOWERS = TWEETS.FK_USER WHERE FOLLOW.FK_USER =?  LIMIT 25',(username))
    conn.commit()
    cur.close()
    conn.close()
...
def postTweet():
    ...
    conn = sqlite3.connect('data.db')
    cur = conn.cursor()
    cur.execute('INSERT INTO TWEETS VALUES (USERNAME, TEXT, DATE) VALUES(?,?,?)', (Username, tweetText, date))
    conn.commit()
    cur.close()
    conn.close()
